...
function parseSingleHeader(
  view: DataView,
  startOffset: number
): { header: FitsHeader; headerBytes: number } {
...
export function getStandardHeader(hdu: FitsHDU): StandardFitsHeader {
  ...
  // Stack / combine count
  const stackCount = Number(h.NCOMBINE ?? h.STACKCNT ?? h.SNAPSHOT ?? h.NEXP ?? h.NFRAMES ?? 1);
  ...
  const totalexp = Number(h.TOTALEXP ?? h['TOTAL-EXP'] ?? h['TOTEXP'] ?? exptime * stackCount);
